...
import com.arandarkt.cache.ArandarCache
...
class LoginDecoder : ByteToMessageDecoder() {
    override fun decode(ctx: ChannelHandlerContext, buffer: ByteBuf, out: MutableList<Any>) {
        ...
        var loginEncryptSize = loginSize - 69
        ...
        val lowMemory = buffer.readBoolean()

        val cache = ArandarCache.cache()

        for (i in 0 until cache.indices().size) {
            val crc = cache.index(i).crc
            if (crc != buffer.readInt() && crc != 0) {
                out.add(LoginResponse.UPDATED.toLoginRequest(session.sessionKey))
                return
            }
        }
        ...
        println(secureId)
        if (secureId != 10) {
            out.add(LoginResponse.COULD_NOT_LOGIN)
            return
        }
